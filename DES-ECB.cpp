...
std::vector<int> DES(std::vector<int> encrypted, std::vector<int> key,int round, bool flag);	//Раунд шифрования DES
...
std::string encryption(std::string encrypted, std::string key)
{
	...
	if (vecEncrypted.size() / 8 == 1)
	{
		...
		for (int i = 0; i < 16; i++)
		{
			...
			vecCipher = DES(vecCipher, subKey, 1, true);						//Раунд шифрования
		}
	...
	else
	{
		...
		{
			vecCipher.push_back(vecEncrypted[i]);
			if (counter == 7)
			{
				vecCipher = permutation(vecCipher, true);	//Начальная перестановка блока
				for (int i = 0; i < 16; i++)
...
std::string decryption(std::string cipher, std::string key)
{
	...
	{
		vecCipher.push_back(vecEcnrypted[i]);
		if (counter == 7)
		{
			vecCipher = permutation(vecCipher, true);	//Конечная перестановка блока
			for (int i = 0; i < 16; i++)
			{
				std::vector<int>subKey = cyclic_permutation(vecKey, 15 - i);	//Получение подключа на раунд
				vecCipher = DES(vecCipher, subKey, 0, false);							//Раунд шифрования
			}

			vecCipher = permutation(vecCipher, false);	//Начальная перестановка
...
std::vector<int> DES(std::vector<int> encrypted, std::vector<int> key, int round, bool flag)
{
	std::vector<int> resF;					
	...
	if (flag)
		{
			R = s_box(six_bit);
		}					//Прохождение 6 битных блоков через S боксы
	else
	{
		L = s_box(six_bit);
	}
	...
	if (flag) R = XOR(R, L);								//XOR левой части и правой, прошедшей через функцию
	else L = XOR(L, R);

	//Соединение блоков в один
	
	if (flag) for (int i = 0; i < R.size(); i++)
		next.push_back(R[i]);
	else for (int i = 0; i < next.size(); i++)
		L.push_back(next[i]);
